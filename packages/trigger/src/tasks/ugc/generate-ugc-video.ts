...
export const generateUgcVideo = schemaTask({
  ...
  run: async (payload, { ctx }) => {
    ...
    try {
      ...
      const plannerValue =
        clip.models?.planner ||
        project.models.planner ||
        clip.models?.script ||
        project.models.script;
      const planner = plannerValue
        ? await loadModel(plannerValue).catch(() => null)
        : null;
      ...
      if (!payload.skipPlanner && !payload.plannedPrompt && planner) {
        setGenerationStatus(
          20,
          influencer ? `Planning ${influencer.name}` : "Planning clip",
        );
        try {
      ...
      if (audioUrl) {
        ...
        if (audioMode === "lip-sync") {
          ...
          try {
            ...
            logger.error("UGC lip-sync failed, muxing voiceover", {
              clipId: clip.id,
              error,
            });
      ...
      const videoTakes = appendUgcVideoTakes(clip.videoTakes ?? [], {
        id: ctx.run.id,
