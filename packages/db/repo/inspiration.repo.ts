...
export const getInspirationCategories = async (query: string) => {
  ...
  const [categories, total] = await Promise.all([
    InspirationCategoryModel.find(match).sort(sort).skip(pagination.skip).limit(pagination.limit).lean(),
...
export const getInspirationNiches = async (query: string) => {
  ...
  const [niches, total] = await Promise.all([
    InspirationNicheModel.find(match).sort(sort).skip(pagination.skip).limit(pagination.limit).lean(),
