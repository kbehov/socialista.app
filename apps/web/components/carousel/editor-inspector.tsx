    ...
    return (
      <div className="shrink-0 space-y-2 border-b px-3 py-2.5">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground">Background, layers, text, and images</p>
        </div>
        <div className="flex gap-0.5 rounded-md border border-border/50 bg-muted/30 p-0.5">
          {TABS.map(({ id, label, icon: Icon }) => (
            <Button
              ...
              className={cn(
                'h-7 flex-1 gap-1 rounded-sm px-1 text-[11px] font-medium',
                tab === id
      ...
      <div className="flex gap-0.5 rounded-lg border border-border/50 bg-muted/40 p-0.5">
        {TABS.map(({ id, label, icon: Icon }) => (
          <Button
            ...
            onClick={() => onTabChange(id)}
            className={cn(
              'h-7 flex-1 gap-1 rounded-md px-1.5 text-[11px] font-medium',
              tab === id
                ? 'bg-background text-foreground shadow-xs'
                : 'text-muted-foreground hover:bg-background/50 hover:text-foreground',
            )}
...
export function EditorInspector({
  ...
  useEffect(() => {
    ...
    // Keep the layers panel open when selecting from the list; only auto-switch on new selection.
