...
import {
  UgcSceneWorkbench,
  type UgcWorkbenchTab,
} from "@/components/studio/ugc/ugc-scene-workbench";
...
import {
  hasUgcProduct,
  ugcClipGeneratedStills,
  ugcNeedsCreator,
} from "@/lib/studio/ugc/ugc-stage";
import {
  buildCampaignVoicePatches,
  ugcVoiceEquals,
} from "@/lib/studio/ugc/voices";
...
function UgcProjectWorkspaceInner({
  workspaceId,
  initialProject,
  ...
  useEffect(() => {
    if (!influencerParam || influencerImportRef.current === influencerParam)
      return;
  ...
  const handleCreateClip = async (
    type: UgcClipType = UGC_DEFAULT_CLIP_TYPE,
  ) => {
  ...
  const applyClipAudio = useCallback(
    (url: string) => {
      ...
      const take = ugcClipAudioTakeForUrl(
  ...
  const applyClipVideo = useCallback(
    ...
    [flushDebounced, patchClip, patchClipLocal, selectedClip],
  );
  ...
  const persistCampaignVoice = useCallback(
    async (voice: UgcClipVoice, clipId?: string) => {
      ...
      if (clipId && patches.clipPatch)
        await patchClip(clipId, patches.clipPatch);
    },
  ...
  const applyStillUrls = (urls: string[]) => {
    ...
    const picked = urls.map((url, index) => {
      const existing = selectedClip.stills.find(
        (still) => still.imageUrl === url,
      );
      return {
        index,
        imageUrl: url,
  ...
  return (
    <div className="ugc-studio flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-background text-foreground">
      ...
      {activeRuns.map((run) => (
        <UgcRunWatcher
          ...
          onProgress={(progress, label) =>
            updateRunProgress(run.key, progress, label)
          }
          onSettled={() => {
            settleRun(run);
      ...
      <UgcStudioTopbar
        ...
        finishing={openingProjectEditor}
        canFinish={allScenesReady && !anyGenerating}
        editorPrepared={Boolean(project.composedProjectVideoId)}
        generating={anyGenerating || generatingAudio || writingScript}
        settingsIncomplete={settingsIncomplete}
        onNameChange={handleNameChange}
        onFinish={handleOpenEditor}
        ...
        onGenerateAllVideos={() => generateAllVideos(project.clips)}
      />
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col lg:flex-row">
        <UgcClipRail
          ...
          onReorder={(clipOrder) => {
            patchProjectLocal({
              clips: clipOrder.flatMap((id) =>
                project.clips.filter((clip) => clip.id === id),
              ),
            });
        ...
        <UgcSceneWorkbench
          ...
          onWriteScript={() => {
            ...
            startWriteScript(async () => {
              const response = await generateUgcClipScript(
                project.id,
                selectedClip.id,
                {
          ...
          onVoiceChange={(voice) => {
            if (!selectedClip) return;
            if (
              ugcVoiceEquals(ugcResolvedClipVoice(project, selectedClip), voice)
            )
              return;
        ...
        <UgcSettingsSidebar
          ...
          applyingPreset={applyingPreset}
          busy={generatingStill || generatingVideo}
          className={cn(
