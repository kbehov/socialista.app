...
import { landingMockupLabel } from "../landing-classes";
...
function Inspector() {
  return (
    <>
      <div>
        <p className={landingMockupLabel}>Channels</p>
        <p className="mt-1.5 text-xs font-medium">IG · TikTok · LinkedIn</p>
      </div>
      <div>
        <p className={landingMockupLabel}>Status</p>
        <p className="mt-1.5 text-xs font-medium">Scheduled · Tue 9:00</p>
      </div>
      <div>
        <p className={landingMockupLabel}>Variants</p>
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-2 rounded-[calc(var(--radius)-2px)] border border-border bg-surface-0 px-3 py-2 text-[0.6875rem] text-muted-foreground">
          <span className="truncate">
            New drop — swipe for the full story →
          </span>
        </div>
        <div className="flex gap-2">
          <MediaFrame
            src={IMG.canvasFashion}
        ...
        <div className="grid grid-cols-7 gap-px overflow-hidden rounded-[calc(var(--radius)-2px)] border border-border bg-border">
          {DAYS.map((day, i) => (
            <div
              key={day}
              className="min-h-[3.25rem] bg-background p-[0.35rem] text-[0.625rem] text-muted-foreground"
            >
              <span>{day}</span>
              {EVENTS[i]?.map((event) => (
                <div
