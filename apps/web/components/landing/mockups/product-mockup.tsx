...
export function ProductMockup({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "grid min-h-[22rem] grid-cols-[2.75rem_1fr] md:min-h-[26rem]",
          inspector
            ? "md:grid-cols-[11rem_1fr_10rem]"
      ...
      >
        <aside className="flex flex-col gap-1.5 border-r border-border bg-background p-2 px-2 py-3">
          ...
          <p className={`${landingMockupLabel} hidden px-2 py-1 md:block`}>
            Studio
          </p>
          {navItems.map((item) => {
        ...
        {inspector ? (
          <aside className="hidden flex-col gap-3 border-l border-border bg-background p-3 md:flex">
            {inspector}
          </aside>
        ) : null}
      </div>
