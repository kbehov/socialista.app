...
function Inspector() {
  return (
    <>
      <div>
        <p className={landingMockupLabel}>Range</p>
        <p className="mt-1.5 text-xs font-medium">Last 30 days</p>
      </div>
      <div>
        <p className={landingMockupLabel}>Top channel</p>
        <p className="mt-1.5 text-xs font-medium">Instagram</p>
      </div>
      <div>
        <p className={landingMockupLabel}>Published</p>
    ...
    >
      <div className="space-y-3">
        ...
        <div className="space-y-1.5">
          {["Summer launch carousel", "UGC hook v2", "Static ad — mug"].map(
            (run, i) => (
              <div
                key={run}
