...
async function loadAuthedProject(
  projectId: string,
): Promise<
...
async function triggerStills(payload: {
  ...
  const handle = await tasks.trigger<GenerateUgcStillsTask>(
    TASK_IDS.generateUgcStills,
    payload,
  );
...
async function triggerAudio(payload: {
  ...
  const handle = await tasks.trigger<GenerateUgcAudioTask>(
    TASK_IDS.generateUgcAudio,
    payload,
  );
  const publicAccessToken = await createPublicAccessToken(handle.id);
  return { success: true, runId: handle.id, publicAccessToken };
...
export async function startUgcStillsGeneration(input: {
  ...
  try {
    ...
    const referenceImageUrls = input.referenceImageUrls?.filter(
      (url) => url.length > 0,
    );
    const count = input.count
      ? clampImageGenerationCount(input.count)
      : undefined;
    ...
    return fail(
      error instanceof Error ? error.message : "Could not generate photos",
    );
  }
}
...
export async function startUgcAudioGeneration(input: {
  ...
  try {
    ...
    if (clips.length === 0) {
      return fail(
        input.clipId ? "Clip not found" : "Add a talking scene first",
      );
    }
    ...
    for (const clip of clips) {
      ...
      const text =
        (input.clipId === clip.id ? incoming : undefined) ||
        clip.script?.text.trim();
      if (!text) {
    ...
    const scriptText = input.clipId
      ? incoming || targets[0]?.script?.text.trim()
      : undefined;
