...
type UseStaticAdGenerationRunOptions = {
  runId: string
  accessToken: string | null
}

export function useStaticAdGenerationRun({ runId, accessToken }: UseStaticAdGenerationRunOptions) {
  ...
  const {
    ...
    error: realtimeError,
    stop,
  } = useRealtimeRun<RealtimeStaticAdGenerationTask>(runId, {
    ...hookOptions,
    enabled: enabled && realtimeEnabled,
  })

  const { run: polledRun, error: polledError } = useRun<RealtimeStaticAdGenerationTask>(runId, {
    ...hookOptions,
