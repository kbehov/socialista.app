...
/** Breathing room for scroll padding, artboard frame, and shadow inside the workspace. */
export const VIDEO_PREVIEW_VIEWPORT_CHROME = 24

/** Slight shrink so the artboard stays inside the preview container at 100% zoom. */
export const VIDEO_PREVIEW_FIT_SCALE = 0.98

/** Space reserved above/below the artboard for floating preview toolbars. */
export const VIDEO_PREVIEW_TOOLBAR_CHROME_Y = 56

/** Horizontal inset so the artboard does not touch panel edges. */
export const VIDEO_PREVIEW_TOOLBAR_CHROME_X = 16
...
export function fitVideoPreviewInWorkspace(
  ...
  canvasWidth: number,
  canvasHeight: number,
  options?: { reserveToolbarChrome?: boolean; rulerGutter?: number },
): { width: number; height: number } {
  const reserveToolbar = options?.reserveToolbarChrome ?? true
  const rulerGutter = options?.rulerGutter ?? 0
  const chromeX =
    VIDEO_PREVIEW_VIEWPORT_CHROME +
    (reserveToolbar ? VIDEO_PREVIEW_TOOLBAR_CHROME_X : 0) +
    rulerGutter
  const chromeY =
    VIDEO_PREVIEW_VIEWPORT_CHROME +
    (reserveToolbar ? VIDEO_PREVIEW_TOOLBAR_CHROME_Y : 0) +
    rulerGutter

  const effectiveHeight = Math.max(workspaceHeight - chromeY, 1)
  const effectiveWidth = Math.max(workspaceWidth - chromeX, 1)
  ...
  return {
    width: Math.round(fit.width * VIDEO_PREVIEW_FIT_SCALE),
    height: Math.round(fit.height * VIDEO_PREVIEW_FIT_SCALE),
  }
}
