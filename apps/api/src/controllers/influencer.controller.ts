...
function parseDirections(value: unknown): string | undefined {
  ...
  if (trimmed.length > DIRECTIONS_MAX) {
    throw new HttpError(400, `Directions must be ${DIRECTIONS_MAX} characters or fewer`)
  }
  return trimmed
}
...
function parseImageUrls(value: unknown, label: string, min: number, max?: number): string[] {
  ...
  const urls = value.filter((item): item is string => typeof item === 'string').map(item => item.trim()).filter(Boolean)
  if (urls.length < min) {
...
async function bestEffortDeleteMedia(urls: string[], workspaceId: string | null) {
  ...
  for (const image of images) {
    try {
      ...
      console.error('Failed to delete influencer media', { key: image.key, error })
    }
  }
}
...
export const getWorkspaceInfluencers = async (c: Context<AppContext>) => {
  ...
  const params = new URLSearchParams(withQueryParam(c.req.url, 'workspace', workspaceId))
  if (!params.has('limit')) params.set('limit', DEFAULT_EXPLORE_LIMIT)
...
export const getInfluencer = async (c: Context<AppContext>) => {
  ...
  return successResponse(c, 200, { influencer: serializeInfluencer(influencer) })
}

export const createInfluencer = async (c: Context<AppContext>) => {
  const userId = c.get('userId')
  const body = (await c.req.json()) as CreateInfluencerPayload & Record<string, unknown>
  ...
  const handle = await tasks.trigger<GenerateInfluencerTask>(TASK_IDS.generateInfluencer, {
...
export const cloneInfluencer = async (c: Context<AppContext>) => {
  ...
  const uploadedImageUrls = parseImageUrls(body.uploadedImageUrls, 'uploadedImageUrls', MIN_CLONE_PHOTOS)
  const name = requireTrimmedString(body.name, 'Name')
  const gender = parseGender(body.gender)
  const ageRange = parseAgeRange(body.ageRange)
  ...
  const handle = await tasks.trigger<CloneInfluencerTask>(TASK_IDS.cloneInfluencer, {
...
export const getCloneRequest = async (c: Context<AppContext>) => {
  ...
  return successResponse(c, 200, { cloneRequest: serializeCloneRequest(request) })
}
...
export const updateInfluencer = async (c: Context<AppContext>) => {
  ...
  const body = (await c.req.json()) as UpdateInfluencerPayload & Record<string, unknown>
  const updates: {
    name?: string
    bio?: string | null
